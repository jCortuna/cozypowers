...
Load this file when doing a UI review/audit pass, or when you need the full checklist for a category beyond the priority table in SKILL.md. Each rule is also present verbatim in `data/ux-guidelines.csv` / `data/app-interface.csv` and is reachable by searching `data/ux-guidelines.csv` / `data/app-interface.csv` (see `searching-the-corpus.md`) — this file is a static index for quick scanning without a search round-trip.
...
- `style-match` - Match style to product type (run the design-system search sequence for recommendations)
- `consistency` - Use same style across all pages
- `no-emoji-icons` - Use SVG icons (Heroicons, Lucide), not emojis
- `color-palette-from-product` - Choose palette from product/industry (search `data/colors.csv`)
